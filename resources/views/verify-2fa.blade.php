...
<head>
    ...
    <title>Verifikasi 2FA | CHASTE</title>
        ...
        <div
            ...
            <div class="w-full md:w-[700px] bg-white p-8 flex flex-col justify-center">
                @php
                    ...
                    $isRegister = $registerData !== null;
                @endphp
                
                <h2 class="text-2xl font-bold text-gray-900 mb-3 text-center">
                    {{ $isRegister ? 'VERIFIKASI REGISTRASI' : 'VERIFIKASI 2FA' }}
                </h2>
                ...
                @if ($isRegister && isset($registerData['email']))
                    <p class="text-sm text-gray-500 mb-4 text-center">
                        Email: <strong>{{ $registerData['email'] }}</strong>
                    </p>
                @endif

                <form method="POST" action="{{ route('verify-2fa') }}" class="mt-3">
                    @csrf
                ...
                <div class="text-center text-sm mt-4">
                    @if ($isRegister)
                        <a href="{{ route('register') }}" class="text-red-500 font-medium">Kembali ke Daftar</a>
                    @else
