...
@section('content')
    ...
    <div class="relative overflow-hidden bg-gradient-to-r from-teal-500 to-teal-600 rounded-xl mb-4 sm:mb-6 lg:mb-8">
        ...
        <div class="relative px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-1 sm:mb-2">📦 Dashboard Gudang</h1>
                    <p class="text-sm sm:text-base text-teal-100">Manajemen gudang dan proses produksi</p>
                </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-5 lg:gap-6 mb-4 sm:mb-6 lg:mb-8">
        <div class="bg-white dark:bg-[#1e1e1e] rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Pesanan Siap Diproses</p>
                    <p class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">{{ $siapProsesCount ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-[#1e1e1e] rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Total Produk Perlu Disiapkan</p>
                    <p class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">{{ $totalProdukDisiapkan ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-[#1e1e1e] rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Returan Perlu Diproses</p>
                    <p class="text-xl sm:text-2xl font-bold text-orange-600 dark:text-orange-400 truncate">{{ $returanCount ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-[#1e1e1e] rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Surat Perintah Menunggu</p>
                    <p class="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400 truncate">{{ $workOrderStats['pending'] ?? 0 }}</p>
                </div>
                <div class="p-2 sm:p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex-shrink-0 ml-2">
                    <svg class="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
        ...
        <div class="bg-white dark:bg-[#1e1e1e] rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Tanggal</p>
                    <p class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">{{ now()->format('d M Y') }}</p>
                </div>
    ...
    <div class="bg-white dark:bg-[#1e1e1e] border border-gray-200 dark:border-gray-700 rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg mb-4 sm:mb-6 lg:mb-8">
        <h2 class="text-base sm:text-lg font-semibold text-gray-700 dark:text-gray-300 mb-3 sm:mb-4">Daftar Pesanan Siap Diproses Gudang</h2>
        {{-- Desktop Table View --}}
                ...
                <tbody>
                    @forelse ($orders as $order)
                        <tr class="border-b border-gray-100 dark:border-gray-700 @if($loop->odd) bg-gray-50 dark:bg-[#2c2c2c] @endif hover:bg-teal-50 dark:hover:bg-[#003333] transition">
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-block px-2 sm:px-3 py-1 rounded-full text-xs font-semibold bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300">{{ ucfirst($order->status) }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center text-gray-500 dark:text-gray-400 py-8">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-base sm:text-lg font-medium">Tidak ada pesanan siap diproses</p>
                                    <p class="text-xs sm:text-sm text-gray-400 dark:text-gray-500 mt-1">Pesanan yang siap diproses akan muncul di sini</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            @forelse ($orders as $order)
                <div class="p-3 sm:p-4 hover:bg-teal-50 dark:hover:bg-[#003333] transition">
                    <div class="flex justify-between items-start mb-2">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-mono font-semibold text-teal-700 dark:text-[#80C0CE] truncate">{{ $order->code }}</p>
                            <p class="text-sm text-gray-800 dark:text-gray-200 truncate">{{ $order->customer->name ?? '-' }}</p>
                        </div>
                        <span class="inline-block px-2 py-1 rounded-full text-xs font-semibold bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 ml-2 flex-shrink-0">
                            {{ ucfirst($order->status) }}
                        </span>
                    </div>
                    <p class="text-sm font-semibold text-gray-800 dark:text-gray-200">Rp {{ number_format($order->grand_total, 0, ',', '.') }}</p>
                </div>
            @empty
                <div class="p-4 text-center text-gray-500 dark:text-gray-400">
                    <div class="flex flex-col items-center justify-center">
                        ...
                        <p class="text-base font-medium">Tidak ada pesanan siap diproses</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Pesanan yang siap diproses akan muncul di sini</p>
                    </div>
                </div>
            @endforelse
        </div>
    ...
    <div class="bg-white dark:bg-[#1e1e1e] border border-gray-200 dark:border-gray-700 rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg mb-4 sm:mb-6 lg:mb-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
            <h2 class="text-base sm:text-lg font-semibold text-gray-700 dark:text-gray-300">Returan Perlu Diproses</h2>
            <div class="flex flex-wrap gap-2 w-full sm:w-auto">
                <a href="{{ route('gudang.stok-barang') }}" class="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md text-xs sm:text-sm font-semibold transition shadow-sm">
                    ...
                    <span class="hidden sm:inline">Stok Barang</span>
                    <span class="sm:hidden">Stok</span>
                </a>
                <a href="{{ route('gudang.laporan-stok') }}" class="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-green-500 hover:bg-green-600 text-white rounded-md text-xs sm:text-sm font-semibold transition shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3 h-3 sm:w-4 sm:h-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h11.25c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
                    </svg>
                    <span class="hidden sm:inline">Laporan Stok</span>
                    <span class="sm:hidden">Laporan</span>
                </a>
                <a href="{{ route('gudang.barang-rusak') }}" class="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-md text-xs sm:text-sm font-semibold transition shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3 h-3 sm:w-4 sm:h-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <span class="hidden sm:inline">Barang Rusak</span>
                    <span class="sm:hidden">Rusak</span>
                </a>
            </div>
        </div>
        {{-- Desktop Table View --}}
                        ...
                        <tr>
                            <td colspan="5" class="text-center text-gray-500 dark:text-gray-400 py-8">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-base sm:text-lg font-medium">Tidak ada returan perlu diproses</p>
                                    <p class="text-xs sm:text-sm text-gray-400 dark:text-gray-500 mt-1">Returan yang perlu diproses akan muncul di sini</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="md:hidden divide-y divide-gray-200 dark:divide-gray-700">
            @forelse ($returans as $retur)
                <div class="p-3 sm:p-4 hover:bg-orange-50 dark:hover:bg-[#003333] transition">
                    <div class="flex justify-between items-start mb-2">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-mono font-semibold text-orange-700 dark:text-orange-400 truncate">#{{ $retur->id }}</p>
                            <p class="text-sm text-gray-800 dark:text-gray-200 truncate">{{ $retur->invoice->customer->name ?? '-' }}</p>
                        </div>
                    ...
                    <div class="mb-2">
                        ...
                        <p class="text-sm font-mono font-semibold text-teal-700 dark:text-[#80C0CE]">{{ $retur->invoice->code ?? '-' }}</p>
                    </div>
                    <p class="text-xs text-gray-600 dark:text-gray-400">{{ $retur->created_at->format('d M Y H:i') }}</p>
                </div>
            @empty
                <div class="p-4 text-center text-gray-500 dark:text-gray-400">
                    <div class="flex flex-col items-center justify-center">
                        ...
                        <p class="text-base font-medium">Tidak ada returan perlu diproses</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Returan yang perlu diproses akan muncul di sini</p>
                    </div>
                </div>
            @endforelse
        </div>
    ...
    <div class="bg-white dark:bg-[#1e1e1e] border border-gray-200 dark:border-gray-700 rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
            <h2 class="text-base sm:text-lg font-semibold text-gray-700 dark:text-gray-300">Surat Perintah Kerja</h2>
            <a href="{{ url('/gudang/work-orders') }}" class="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md text-xs sm:text-sm font-semibold transition shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3 h-3 sm:w-4 sm:h-4">
                    ...
                    @forelse ($workOrders as $workOrder)
                        <tr class="border-b border-gray-100 dark:border-gray-700 @if($loop->odd) bg-gray-50 dark:bg-[#2c2c2c] @endif hover:bg-blue-50 dark:hover:bg-[#003333] transition">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @endphp
                                <span class="inline-block px-2 sm:px-3 py-1 rounded-full text-xs font-semibold {{ $statusColor }}">
                                    {{ $statusLabel }}
                                </span>
                            </td>
                            <td class="px-4 py-3">
                        ...
                        <tr>
                            <td colspan="6" class="text-center text-gray-500 dark:text-gray-400 py-8">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-base sm:text-lg font-medium">Tidak ada surat perintah kerja yang ditugaskan</p>
                                    <p class="text-xs sm:text-sm text-gray-400 dark:text-gray-500 mt-1">Surat perintah kerja yang ditugaskan akan muncul di sini</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="md:hidden divide-y divide-gray-200 dark:divide-gray-700">
            @forelse ($workOrders as $workOrder)
                @php
                    ...
                    $statusLabel = $statusLabels[$workOrder->status] ?? ucfirst($workOrder->status);
                @endphp
                <div class="p-3 sm:p-4 hover:bg-blue-50 dark:hover:bg-[#003333] transition">
                    <div class="flex justify-between items-start mb-2">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-mono font-semibold text-blue-700 dark:text-blue-400 truncate">{{ $workOrder->code }}</p>
                            <p class="text-sm text-gray-800 dark:text-gray-200 truncate">{{ $workOrder->createdBy->name ?? '-' }}</p>
                        </div>
                    ...
                    <div class="mb-2">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <p class="text-xs text-gray-600 dark:text-gray-400">{{ $progressPercentage }}%</p>
                        </div>
                    ...
                    <div class="flex justify-between items-center mt-3">
                        <p class="text-xs text-gray-600 dark:text-gray-400">{{ $workOrder->created_at->format('d M Y H:i') }}</p>
                        <a href="{{ url('/gudang/work-orders/' . $workOrder->id) }}" 
                           class="inline-block px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-md text-xs font-semibold transition shadow-sm">
                            Detail
                        </a>
                    </div>
                </div>
            @empty
                <div class="p-4 text-center text-gray-500 dark:text-gray-400">
                    <div class="flex flex-col items-center justify-center">
                        ...
                        <p class="text-base font-medium">Tidak ada surat perintah kerja yang ditugaskan</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Surat perintah kerja yang ditugaskan akan muncul di sini</p>
                    </div>
                </div>
            @endforelse
        </div>
