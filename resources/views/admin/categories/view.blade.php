...
@section('content')
    ...
    <div class="relative overflow-hidden bg-gradient-to-r from-teal-500 to-teal-600 rounded-xl mb-4 sm:mb-6 lg:mb-8">
        ...
        <div class="relative px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-1 sm:mb-2">🏷️ Kelola Kategori</h1>
                    <p class="text-sm sm:text-base text-teal-100">Kelola kategori produk dan organisasi</p>
                </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6 mb-4 sm:mb-6 lg:mb-8">
        <div class="bg-white rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xl sm:text-2xl font-bold text-gray-900 truncate">{{ $categories->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-gray-600">Kategori Aktif</p>
                    <p class="text-xl sm:text-2xl font-bold text-emerald-600 truncate">{{ $categories->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xl sm:text-2xl font-bold text-blue-600 truncate">{{ $categories->sum('products_count') ?? 0 }}</p>
                </div>
        ...
        <form method="GET" action="" class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-center gap-3 sm:gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <label for="search" class="text-xs sm:text-sm font-semibold text-gray-700">Cari Kategori:</label>
                </div>
            </div>
            <div class="flex flex-wrap items-center gap-2 sm:gap-3 w-full lg:w-auto">
                <div class="relative flex-1 sm:flex-none">
                    ...
                    <input type="text" name="search" placeholder="Cari nama kategori..." value="{{ request('search') }}" class="pl-10 pr-4 py-2 w-full sm:w-64 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all duration-200 shadow-sm text-sm" />
                </div>
                ...
                @if(request('search'))
                    <a href="{{ url('/admin/categories') }}" class="px-4 py-2 bg-gray-100 text-gray-700 text-sm sm:text-base font-medium rounded-lg hover:bg-gray-200 transition-all duration-200 w-full sm:w-auto text-center">Reset</a>
                @endif
            </div>
        </form>
    </div>
        ...
        <div class="hidden lg:block overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">#</th>
                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nama Kategori</th>
                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Jumlah Produk</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($categories as $index => $category)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-8 w-8 sm:h-10 sm:w-10">
                                        <div class="h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-gradient-to-r from-teal-400 to-teal-600 flex items-center justify-center">
                                            <span class="text-white font-semibold text-xs sm:text-sm">{{ substr($category->name, 0, 1) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-2 sm:ml-4">
                                        <div class="text-xs sm:text-sm font-semibold text-gray-900 truncate">{{ $category->name }}</div>
                                        <div class="text-xs sm:text-sm text-gray-500 truncate">Kategori Produk</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                            ...
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800">
                            ...
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex flex-wrap gap-2">
                                    <a href="{{ route('admin.categories.detail', $category->id) }}"
                                       class="inline-flex items-center gap-1 px-2 sm:px-3 py-1.5 sm:py-2 border border-teal-600 text-teal-600 text-xs sm:text-sm font-medium rounded-lg hover:bg-teal-50 transition-all duration-200 shadow-sm">
                        ...
                        <tr>
                            <td colspan="6" class="px-4 sm:px-6 py-8 sm:py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-2">Tidak ada data kategori</h3>
                                    <p class="text-sm sm:text-base text-gray-500 mb-4">Mulai dengan menambahkan kategori baru</p>
                                    <button onclick="showAddForm()" class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-500 to-teal-600 text-white text-sm sm:text-base font-medium rounded-lg hover:from-teal-600 hover:to-teal-700 transition-all duration-200 shadow-lg">
                ...
                <div class="p-4 hover:bg-gray-50 transition-colors duration-200">
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex items-center flex-1 min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-600 truncate">Kategori Produk</p>
                            </div>
                        </div>
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 ml-2 flex-shrink-0">
                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                            Aktif
                    ...
                    <div class="space-y-2 mb-3 text-xs sm:text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-900">{{ $category->created_at ? $category->created_at->format('d M Y') : '-' }}</span>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        <a href="{{ route('admin.categories.detail', $category->id) }}"
                           class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 border border-teal-600 text-teal-600 text-xs sm:text-sm font-medium rounded-lg hover:bg-teal-50 transition-all duration-200 shadow-sm">
                            <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            Detail
    ...
    <div id="addModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 p-4">
        <div class="bg-white rounded-xl p-4 sm:p-6 w-full max-w-md shadow-2xl">
            <h2 class="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-gray-900">Tambah Kategori</h2>
            <form action="{{ route('admin.categories.store') }}" method="POST">
                @csrf
                <input type="text" name="name" placeholder="Nama Kategori"
                    class="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 mb-3 sm:mb-4 transition-all duration-200 text-sm sm:text-base" required>
                <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                    <button type="button" onclick="hideAddForm()"
                        class="px-3 sm:px-4 py-2 bg-gray-100 text-gray-700 text-sm sm:text-base font-medium rounded-lg hover:bg-gray-200 transition-all duration-200 w-full sm:w-auto">
                        Batal
                    </button>
                    <button type="submit" class="px-3 sm:px-4 py-2 bg-gradient-to-r from-teal-500 to-teal-600 text-white text-sm sm:text-base font-medium rounded-lg hover:from-teal-600 hover:to-teal-700 transition-all duration-200 shadow-lg w-full sm:w-auto">
                        Simpan
                    </button>
                </div>
            </form>
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 p-4">
        <div class="bg-white rounded-xl p-4 sm:p-6 w-full max-w-md shadow-2xl">
            <h2 class="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-gray-900">Edit Kategori</h2>
