...
<form action="{{ route('admin.categories.store') }}" method="POST" class="mb-6 flex gap-4">
    ...
    <input type="text" name="name" placeholder="Nama Kategori"
           class="border border-gray-300 rounded px-4 py-2 w-full max-w-sm focus:outline-teal-600"
           required>
    <button type="submit"
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white rounded p-6 w-full max-w-md">
        ...
        <form id="editForm" method="POST">
            @csrf
            @method('POST')
            <input type="text" name="name" id="editName" class="border border-gray-300 w-full p-2 rounded mb-4" required>
