...
@section('content')
    ...
    <div class="relative overflow-hidden bg-gradient-to-r from-teal-500 to-teal-600 rounded-xl mb-4 sm:mb-6 lg:mb-8">
        ...
        <div class="relative px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-1 sm:mb-2">👥 Daftar Pegawai</h1>
                    <p class="text-sm sm:text-base text-teal-100">Kelola data pegawai dan tim Anda</p>
                </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6 mb-4 sm:mb-6 lg:mb-8">
        <div class="bg-white rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xl sm:text-2xl font-bold text-gray-900 truncate">{{ $products->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-gray-600">Admin</p>
                    <p class="text-xl sm:text-2xl font-bold text-blue-600 truncate">{{ $products->where('role', 'admin')->count() }}</p>
                </div>
                <div class="p-2 sm:p-3 bg-blue-100 rounded-lg flex-shrink-0 ml-2">
                    <svg class="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-xs sm:text-sm font-medium text-gray-600">Driver</p>
                    <p class="text-xl sm:text-2xl font-bold text-green-600 truncate">{{ $products->where('role', 'driver')->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xl sm:text-2xl font-bold text-emerald-600 truncate">{{ $products->where('active', true)->count() }}</p>
                </div>
        ...
        <form method="GET" action="{{ url('/admin/employees') }}" class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 w-full lg:w-auto">
                <div class="flex items-center gap-2">
                    ...
                    <label for="role" class="text-xs sm:text-sm font-semibold text-gray-700">Filter Role:</label>
                </div>
                <select name="role" id="role" onchange="this.form.submit()" class="px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all duration-200 bg-white shadow-sm w-full sm:w-auto text-sm">
            ...
            <div class="flex flex-wrap items-center gap-2 sm:gap-3 w-full lg:w-auto">
                <div class="relative flex-1 sm:flex-none">
                    ...
                    <input type="text" name="search" placeholder="Cari nama/email pegawai..." value="{{ request('search') }}" class="pl-10 pr-4 py-2 w-full sm:w-64 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all duration-200 shadow-sm text-sm" />
                </div>
                ...
                @if(request('search') || request('role'))
                    <a href="{{ url('/admin/employees') }}" class="px-4 py-2 bg-gray-100 text-gray-700 text-sm sm:text-base font-medium rounded-lg hover:bg-gray-200 transition-all duration-200 w-full sm:w-auto text-center">Reset</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">KTP</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($products as $employee)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $employee->id }}</td>
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-8 w-8 sm:h-10 sm:w-10">
                                        <div class="h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-gradient-to-r from-teal-400 to-teal-600 flex items-center justify-center">
                                            <span class="text-white font-semibold text-xs sm:text-sm">{{ substr($employee->name, 0, 1) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-2 sm:ml-4">
                                        <div class="text-xs sm:text-sm font-semibold text-gray-900 truncate">{{ $employee->name }}</div>
                                        <div class="text-xs sm:text-sm text-gray-500 truncate">{{ $employee->email }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-600 truncate">{{ $employee->email }}</td>
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-600 truncate">{{ $employee->phone }}</td>
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-600 truncate">{{ $employee->ktp }}</td>
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                        ...
                        <tr>
                            <td colspan="8" class="px-4 sm:px-6 py-8 sm:py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-2">Tidak ada data pegawai</h3>
                                    <p class="text-sm sm:text-base text-gray-500 mb-4">Mulai dengan menambahkan pegawai baru</p>
                                    <a href="{{ url('/admin/employees/create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-500 to-teal-600 text-white text-sm sm:text-base font-medium rounded-lg hover:from-teal-600 hover:to-teal-700 transition-all duration-200 shadow-lg">
                ...
                <div class="p-4 hover:bg-gray-50 transition-colors duration-200">
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex items-center flex-1 min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-600 truncate">{{ $employee->email }}</p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-2 mb-3 text-xs sm:text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-900 font-medium truncate">{{ $employee->phone }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">KTP:</span>
                            <span class="text-gray-900 font-medium truncate">{{ $employee->ktp }}</span>
                        </div>
                    </div>
                    <a href="{{ url('/admin/employees/detail/' . $employee->id) }}"
